    ...
    return (
        <section className="relative bg-white" aria-label="Countdown zur Unternehmerbörse" data-navbar="light">
            ...
            <div
                ...
                style={{
                    background:
                        'linear-gradient(90deg, var(--color-surface) 0%, #FE3D4E 72%, #FE3D4E 100%)',
                }}
                aria-hidden="true"
            />
            ...
            <div className="max-w-[1320px] mx-auto px-6 lg:px-12 py-24 md:py-36 text-center flex flex-col items-center">
                {/* Heading */}
                <p tabIndex={0} className="text-[#050505] text-lg md:text-xl font-bold tracking-wide uppercase focus-ring rounded inline-block px-2">
                    Sei dabei am
                </p>
                ...
                {showPassed ? (
                    <div className="mt-12">
                        <p className="text-[#050505]/70 text-lg font-medium rounded-lg p-2 inline-block">
                    ...
                    >
                        ...
                        <span className="text-4xl md:text-6xl text-[#050505]/60 font-black leading-none animate-pulse-fast select-none" aria-hidden="true">:</span>
            ...
            <div
                ...
                style={{
                    background:
                        'linear-gradient(90deg, #FE3D4E 0%, #FE3D4E 28%, var(--color-surface) 100%)',
                }}
                aria-hidden="true"
            />
...
function CountdownUnit({ value, label }: { value: number; label: string }) {
    return (
        <div tabIndex={0} className="relative text-center rounded-xl p-3 focus-ring flex flex-col items-center" aria-label={`${value} ${label}`}>
            <div className="text-5xl md:text-7xl lg:text-8xl font-black text-[#050505] leading-none tabular-nums">
                {String(value).padStart(2, '0')}
            </div>
            <div className="absolute top-full left-1/2 -translate-x-1/2 text-xs md:text-sm font-bold uppercase tracking-widest text-[#050505]/70 mt-2 whitespace-nowrap">
                {label}
            </div>
        </div>
    );
