...
const tierContent: Record<number, { title: string; textColor: string; borderColor: string }> = {
  1: { title: 'Basis Ticket', textColor: 'text-yellow-500', borderColor: 'border-yellow-500' },
  2: { title: 'Basis Plus Ticket', textColor: 'text-blue-500', borderColor: 'border-blue-500' },
  3: { title: 'Premium Ticket', textColor: 'text-red-500', borderColor: 'border-red-500' },
  4: { title: 'Premium Deluxe Ticket', textColor: 'text-emerald-400', borderColor: 'border-emerald-400' },
};
...
export function BookingStatus({ bookings }: BookingStatusProps) {
  ...
  return (
    <section aria-labelledby="booking-status-heading" className="bg-white pb-6 border-t-4 border-[#2860F9]/20 pt-6 space-y-0">
      <h2 id="booking-status-heading" className="text-lg font-bold text-slate-900">
        Ihre Buchung
      </h2>

      {!booking ? (
        <p className="text-sm text-slate-500">Noch keine Buchung vorhanden.</p>
      ) : (
...
function BookedTier({ booking }: { booking: BookingDto }) {
  ...
  return (
    <div className="space-y-1">
      ...
      <div className={`rounded-xl border-2 ${config.borderColor} p-8 bg-surface-raised flex flex-col md:flex-row justify-between items-start md:items-stretch gap-6`}>
        ...
        <div className="flex flex-col justify-between items-end min-w-50 self-stretch text-right gap-6">
          ...
          {booking.tierId < 4 && (
            <Link
              href="/ticketshop"
              className="inline-flex items-center justify-center gap-2 w-full md:w-auto px-6 py-2.5 rounded-lg font-semibold text-sm bg-primary text-primary-foreground transition-transform hover:scale-[1.02] focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-primary"
            >
              Aufwerten
